...
from tools import write_markdown_file, write_text_file
...
def saving_node(state):
    ...
    # save tas a markdown (do not need for now)
    # write_markdown_file(final_doc, f"final_doc_{llm_name}")
    # write_markdown_file(plan, f"plan_{llm_name}")

    # save as txt
    write_text_file(final_doc, f"final_doc_{llm_name}")
    write_text_file(plan, f"plan_{llm_name}")
